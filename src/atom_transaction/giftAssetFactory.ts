...
@Injectable()
export class GiftAssetFactory extends _TransactionFactory<GiftAssetTransaction> {
    readonly GENERATE_API_PATH = GENERATE_TRANSACTION_API_PATH.TR_GIFT_ASSET;

    async generateTransaction(request: TransactionMaker.Transaction.GiftAssetTransactionParams) {
        this.verify(request);

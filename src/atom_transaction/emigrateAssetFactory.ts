...
@Injectable()
export class EmigrateAssetFactory extends _TransactionFactory<EmigrateAssetTransaction> {
    readonly GENERATE_API_PATH = GENERATE_TRANSACTION_API_PATH.TR_EMIGRATE_ASSET;

    async generateTransaction(request: TransactionMaker.Transaction.EmigrateAssetTransactionParams) {
        this.verify(request);
